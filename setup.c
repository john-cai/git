...
{
	if (inside_git_dir < 0)
		inside_git_dir = is_inside_dir(repo_get_git_dir(repo));
...
void setup_work_tree(struct repository *repo)
{
	...
	if (!work_tree || chdir_notify(repo, work_tree))
...
int upgrade_repository_format(struct repository *repo, int target_version)
{
	...
	strbuf_git_common_path(&sb, repo, "config");
...
static void set_git_dir_1(struct repository *repo, const char *path)
{
	...
	setup_git_env(repo, path);
}

static void update_relative_gitdir(struct repository *repo,
...
void set_git_dir(struct repository *repo, const char *path, int make_realpath)
{
	struct strbuf realpath = STRBUF_INIT;
...
void create_reference_database(struct repository *repo, enum ref_storage_format ref_storage_format,
			       const char *initial_branch, int quiet)
{
	...
	repo_set_ref_storage_format(repo, ref_storage_format);
